...
import { FatherHistoryData } from '../types';
import { FIELD_OPTIONS } from '../constants';

interface StepFatherHistoryProps {
  data: FatherHistoryData;
  errors: Record<string, string>;
  onChange: (fieldName: string, value: any) => void;
}

export function StepFatherHistory({ data, errors, onChange }: StepFatherHistoryProps) {
    ...
    <div className="space-y-6">
      <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
        <p className="text-sm text-slate-700">
          We're exploring your father's health history to understand potential generational patterns.
          This includes both emotional and physical health patterns.
        </p>
      </div>
      ...
      <div>
        <label className="block text-sm font-medium text-slate-700 mb-3">
          Did your father experience any of these? (Select all that apply)
        </label>
        ...
        {errors['fatherHistory.emotionalChallenges'] && (
      ...
      <div>
        <label className="block text-sm font-medium text-slate-700 mb-3">
          Did your father experience any chronic health issues? (Select all that apply)
        </label>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
      ...
      <div>
        <label className="block text-sm font-medium text-slate-700 mb-2">
          How would you describe your father's relationship patterns? (optional)
        </label>
        <textarea
