...
const AR_LABELS: Labels = {
  ...
  alreadyRegistered: "أنت مسجّل بالفعل في هذه الفعالية.",
...
export default function EventRegisterButton({ eventId, eventTitle, ctaLabel, locale, className }: Props) {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!res.ok) {
        setStatus("error");
        setErrorMessage(res.status === 409 ? L.alreadyRegistered : data.error || L.genericError);
        return;
      }
