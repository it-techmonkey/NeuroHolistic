...
function BookingModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const isStepValid = useMemo(() => {
    if (currentStep === 1) return formData.name.trim() !== "" && formData.email.includes("@") && formData.phone.trim() !== "" && formData.country.trim() !== "";
  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      ...
      // Create Booking
      // This modal creates consultation bookings (type: "consultation")
      // Program bookings (type: "program") come from a separate flow
      await createBooking({
        name: formData.name,
